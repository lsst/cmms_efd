...
client = EfdQueryClient(site="usdf")
df = client.query('SHOW MEASUREMENTS')
...
def query_latest_influx_value(
    ...
    if sal_index is not None:
        ...
        print(f"Querying InfluxDB: {measurement}.{field} at {datetime.now()}")
        ...
        while True:
            try:
                if type_telemetry == "actuation":
                    ...
                    print("NUMERO DE CONSULTAS", EfdQueryClient.total_queries)
